...
class NN():
    ...
    def Z_Norm(self,z,layer):
        if layer<self.L-2:
            ...
            variance=self.BatchSize*np.var(z,axis=1,keepdims=True)/(self.BatchSize)
            z_normal=(z- self.BN_mu[layer])/(np.sqrt(self.BN_sigma[layer]+1e-7))
            return z_normal
        else:
            return z
    ...
    def B_P(self,Yi,Z,y,A,Z_Batch_Normalization,index):
        ...
        self.gc_dgama=d_BN_gama/self.BatchSize
        self.gc_beta=d_BN_beta/self.BatchSize
        self.GD(dW,d_BN_gama, d_BN_beta)
        # # 权重梯度下降
    ...
    #GD
    def GD(self, dW,d_BN_gama, d_BN_beta):
        for i in range(L-1):
            self.W[i] -= self.alpha*dW[-i-1]
        for i in range(self.L - 2):
            self.BN_beta[i] -= self.alpha *d_BN_beta[-i-1]
            self.BN_gama[i] -= self.alpha *d_BN_gama[-i-1]
    # Adam
    def Adam(self, dW, t):
    ...
    def gc_gama(self, index):
        epslion = 1e-7
        for i in range(0, len(self.BN_gama)):
            dw = np.zeros(self.BN_gama[i].shape)
            for j in range(len(self.BN_gama[i])):
                for k in range(len(self.BN_gama[i][j])):
                    self.BN_gama[i][j][k] -= epslion
                    J_minus = self.gc_FP(index)
                    self.BN_gama[i][j][k] += 2 * epslion
                    J_plus = self.gc_FP(index)
                    dw[j][k] = (J_plus - J_minus) / (2 * epslion)
                    self.BN_gama[i][j][k] -= epslion
            print("W[{i}]: ".format(i=i))
            print(1, "神经网络梯度", self.gc_dgama[-1 - i])
            print(2, "梯度检验", dw)
            print(3, "差", self.gc_dgama[-1 - i] - dw)
            print(4, '差方', (self.gc_dgama[-1 - i] - dw) * (self.gc_dgama[-1 - i] - dw))
            # print(  (self.gc_dw[-1 - i]) * (self.gc_dw[-1 - i]) + (dw) * (dw))
            print(5, (self.gc_dgama[-1 - i] - dw) * (self.gc_dgama[-1 - i] - dw) / (
                        (self.gc_dgama[-1 - i]) * (self.gc_dgama[-1 - i]) + (dw) * (dw)))
            exit()
...
if __name__ == '__main__':
    ...
    struct = [3,8,16,16,16,2,1]
    # super parameters
    L = len(struct)
    alpha = 0.001
    # 创建模型
    model=NN(alpha,struct=struct,BatchSize=10)
    model.Genernate_Train_Data_batch(data_num=500)
    # 训练10次
