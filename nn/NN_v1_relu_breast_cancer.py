...
class NN():
    ...
    def Normalize_Data(self,data):
        average=np.mean(data,axis=1).reshape(data.shape[0],1)
        variance=np.var(data,axis=1).reshape(data.shape[0],1)
        normal_data=(data-average)/(variance)
        return normal_data

    # mini-batch 梯度下降乳腺癌训练集
    def Genernate_Breast_Cancer_Train_Data_batch(self,data_num):
    ...
    def check(self,iteration):
        ...
        for x in range(all):
            if p_y[x] == self.test_y[x]:
                t += 1
        print('第{it}次迭代'.format(it=iteration),'val_acc:', t / len(self.test_y),'val_loss:',J,len(self.test_y),t)
...
if __name__ == '__main__':
    ...
    model.Genernate_Breast_Cancer_Train_Data_batch(data_num=569)
    # 训练10次
