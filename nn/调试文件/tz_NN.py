...
if __name__ == "__main__":
	...
	y_train = np.array([n for n in train_y]).reshape((1,700))
	y_test = np.array([n for n in test_y]).reshape((1,300))
	X_train = np.array([[int(i[1]), int(i[3]), int(i[5])] for i in train_x]).T
	X_test = np.array([[int(i[1]), int(i[3]), int(i[5])] for i in test_x]).T
	print(X_train)
